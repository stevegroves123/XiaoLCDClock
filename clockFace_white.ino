...
void setup() {
  ...
  tft.drawCircle(cenX, cenY, 115, ST77XX_BLACK);
  ticks();
  tft.drawCircle(cenX, cenY, 90, ST77XX_BLACK);
...
void loop() {
  ...
  minutes(min);
  hours(hr);
  tft.fillCircle(cenX, cenY, 5, ST77XX_BLACK);
  delay(500);
...
int Bminutes(int mins) {
  ...
  float xP15 = 5*cos(radian);
  float yP15 = 5*sin(radian);
  tft.drawTriangle(cenX+xM15,cenY+yM15,cenX+x,cenY+y,cenX+xP15,cenY+yP15,ST77XX_WHITE);
}
...
void ticks() {
  for (int i=0; i<360; i=i+30) {
    ...
    for (int b=(tickLength-16); b<(tickLength-14); b++) {
      x=b*cos(radian);
      y=b*sin(radian);
      tft.drawLine(cenX,cenY,cenX+x,cenY+y,ST77XX_WHITE);
    }
  }
}
